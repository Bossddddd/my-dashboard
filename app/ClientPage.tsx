...
export default function Home({
  ...
  const [dashboardSearchResults, setDashboardSearchResults] = useState<
    DashboardSearchResults | null | undefined
  >(undefined);

  const [activeTab, setActiveTab] = useState<
    "dashboard" | "teams" | "technicians" | "map" | "settings"
  >("dashboard");
  ...
  const processedStats = useMemo(() => {
    ...
    if (s.allLogs) {
      const now = new Date();
      s.overdueTasks = s.allLogs.filter(
        (log: any) =>
      ...
      s.mapLogs = s.allLogs.filter(
        (log: any) => log.latitude != null && log.longitude != null,
      );
    }
    return s;
  }, [initialStats]);
  ...
  const [selectedTechnicianDetail, setSelectedTechnicianDetail] = useState<
    any | null
  >(null);
  ...
  const fetcher = async (params: any) => {
    ...
    if (data && data.allLogs) {
      const now = new Date();
      data.overdueTasks = data.allLogs.filter(
        (log: any) =>
          log.status !== "completed" &&
          log.status !== "cancelled" &&
          log.dueDate &&
      ...
      data.mapLogs = data.allLogs.filter(
        (log: any) => log.latitude != null && log.longitude != null,
      );
    }
    return data;
  ...
  const handleTabChange = (
    tab: "dashboard" | "teams" | "technicians" | "settings",
  ) => {
    setActiveTab(tab);
    setVehicleData(undefined);
    setDashboardSearchResults(undefined);
  ...
  const executeSearch = async () => {
    ...
    try {
      ...
      if (searchResults && searchResults.total > 0) {
        toast.success(`พบผลลัพธ์ ${searchResults.total} รายการ`, {
          id: searchToast,
        });
    ...
    if (selectedteam === "all") {
      ...
      stats.teamsData.forEach((w: any) => {
        w.technicians?.forEach((t: any) => {
          const tLogs = w.logs
            ? w.logs.filter((l: any) => l.technicianName === t.name)
            : [];
          if (!allTechsMap.has(t.name)) {
            ...
            existing.efficiencyRate =
              tClosed > 0
                ? Math.round((existing.successCount / tClosed) * 1000) / 10
                : 0;
      ...
      allTechsArray = (wData?.technicians || []).map((t: any) => ({
        ...t,
        logs: wData?.logs
          ? wData.logs.filter((l: any) => l.technicianName === t.name)
          : [],
      }));
    }

    const sortedByEff = [...allTechsArray].sort(
      (a, b) =>
        b.efficiencyRate - a.efficiencyRate || b.successCount - a.successCount,
    );
    const sortedByWorst = [...allTechsArray].sort(
      (a, b) =>
        a.efficiencyRate - b.efficiencyRate || a.lateCount - b.lateCount,
    );
  ...
  const teamSums = useMemo(() => {
    if (!stats?.teamsData)
      return { sumSuccess: 0, sumInProgress: 0, sumLate: 0, totalteams: 0 };
    return {
      totalteams: stats.teamsData.length,
      sumSuccess: stats.teamsData.reduce(
        (acc: number, cur: any) => acc + cur.successCount,
        0,
      ),
      sumInProgress: stats.teamsData.reduce(
        (acc: number, cur: any) => acc + cur.inProgressCount,
        0,
      ),
      sumLate: stats.teamsData.reduce(
        (acc: number, cur: any) => acc + cur.lateCount,
        0,
      ),
    };
  }, [stats]);
  ...
  const renderContent = () => {
    if (!stats)
      return (
        <div className="text-center p-10 text-gray-500 dark:text-slate-400 font-medium flex items-center justify-center gap-2">
          <span className="animate-spin text-xl">⏳</span> กำลังโหลดข้อมูล...
        </div>
      );
    ...
    <LanguageProvider language={language} setLanguage={setLanguage}>
      <div className="flex flex-col h-screen bg-gray-50 dark:bg-slate-900 font-sans overflow-hidden">
        <header className="h-14 sm:h-16 bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between px-3 sm:px-6 flex-shrink-0 z-20 shadow-sm">
          <div className="flex items-center gap-2 sm:gap-4">
            <button
              ...
              className="text-gray-500 dark:text-slate-400 hover:text-gray-800 dark:text-slate-200 p-1.5 rounded-lg hover:bg-gray-100 dark:bg-slate-800/50"
            >
              <svg
            ...
            <div className="flex items-center gap-1 sm:gap-2 mr-1 sm:mr-2">
              <div className="bg-[#0B603A] text-white font-black italic rounded-full px-2 sm:px-3 py-0.5 sm:py-1 text-[10px] sm:text-sm tracking-widest border border-green-200 shadow-xs shrink-0">
                EVT
              </div>
              <span className="font-black text-gray-800 dark:text-slate-200 text-[11px] sm:text-sm hidden sm:block tracking-wide whitespace-nowrap">
                {getTranslation(language, "evtAdminPanel")}
              </span>
            </div>
            ...
            <h1 className="text-xs sm:text-base font-black text-[#0B603A] truncate max-w-[150px] sm:max-w-none">
              {vehicleData !== undefined
                ...
                : selectedteamDetail
                  ? `${getTranslation(language, "teamTitle")} ${selectedteamDetail.name}`
                  : selectedTechnicianDetail
                    ? `${getTranslation(language, "technicianTitle")} ${selectedTechnicianDetail.name}`
                    : activeTab === "dashboard"
                      ? getTranslation(language, "maintenanceDashboard")
                      : activeTab === "teams"
                        ? getTranslation(language, "teams")
                        : activeTab === "technicians"
                          ? getTranslation(language, "technicians")
                          : activeTab === "map"
                            ? "แผนที่รวม"
        ...
        <div className="flex flex-1 overflow-hidden relative">
          ...
          <div
            className={`absolute sm:relative flex-shrink-0 h-full transition-all duration-300 ease-in-out bg-white dark:bg-slate-800 z-30 sm:z-10 shadow-2xl sm:shadow-none ${isSidebarOpen ? "w-64 opacity-100 translate-x-0" : "w-0 opacity-0 -translate-x-full"}`}
          >
