...
const locations = [
  ...
  { label: "สี่แยกปฐมพร ชุมพร", lat: 10.493, lng: 99.18 },
  { label: "สนามบินสุวรรณภูมิ", lat: 13.69, lng: 100.7501 },
  { label: "นิคมอุตสาหกรรมบางปู", lat: 13.535, lng: 100.6439 },
];
...
let csvContent =
  "plate,brand,model,teamName,technicianName,priority,status,category,description,symptoms,locationLabel,latitude,longitude,cost,reportedAt,assignedAt,startedAt,completedAt,dueDate\n";
...
for (let i = 0; i < 100; i++) {
  const province = ["กท", "ชร", "ชบ", "นว", "ขก", "สข"][
  ...
  const dueDate = new Date(
    reportedAt.getTime() + (1 + Math.random() * 4) * 24 * 60 * 60 * 1000,
  );

  if (status !== "reported") {
    assignedAt = new Date(
      reportedAt.getTime() + Math.random() * 4 * 3600 * 1000,
    ).toISOString();
  }
  if (["in_progress", "waiting_parts", "completed"].includes(status)) {
    startedAt = new Date(
      new Date(assignedAt).getTime() + Math.random() * 24 * 3600 * 1000,
    ).toISOString();
  }
  if (status === "completed") {
    completedAt = new Date(
      new Date(startedAt).getTime() + Math.random() * 48 * 3600 * 1000,
    ).toISOString();
