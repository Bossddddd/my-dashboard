  ...
  const confirmDelete = async () => {
    ...
    try {
      if (deleteType === "old") {
        ...
        if (res?.success) {
          toast.success(
            `ลบข้อมูลปี ${selectedYearToDelete} สำเร็จแล้ว (ลบไป ${res.count} รายการ)`,
            { id: loadingToast },
          );
        } else {
        ...
        if (res?.success) {
          toast.success(
            `รีเซ็ตฐานข้อมูลสำเร็จ (ลบใบงาน ${res.logsCount} รายการ, รถ ${res.vehiclesCount} คัน)`,
            { id: loadingToast },
          );
        } else {
          toast.error(res?.error || "เกิดข้อผิดพลาด", { id: loadingToast });
      ...
      <div className="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm">
        <div className="mb-8 border-b border-gray-100 dark:border-slate-700/50 pb-4 flex justify-between items-end">
          <div>
            ...
            <p className="text-gray-500 dark:text-slate-400 font-medium text-sm mt-2">
              {t("settingsDesc")}
            </p>
          </div>
          <button
          ...
          <section>
            ...
            <div className="bg-gray-50 dark:bg-slate-900 rounded-xl border border-gray-100 dark:border-slate-700/50 p-5 space-y-5">
              <div className="flex items-center justify-between">
                ...
                >
                  <div
                    className={`bg-white dark:bg-slate-800 w-5 h-5 rounded-full shadow-md transform transition-transform duration-300 ${isDarkMode ? "translate-x-7" : "translate-x-0"}`}
                  />
                </button>
              </div>
            ...
            <div className="bg-rose-50 dark:bg-rose-900/30 rounded-xl border border-rose-200 dark:border-rose-800 p-5 space-y-5">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex-1">
                  <h4 className="font-bold text-rose-900 dark:text-rose-300 text-sm">
                    {t("deleteByYear")}
                  </h4>
                  <p className="text-xs text-rose-700 dark:text-rose-400/80 font-medium mt-1">
                    {t("deleteByYearDesc")}
                  </p>
                </div>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 max-w-sm w-full shadow-2xl animate-fade-in-up">
            ...
            <h3 className="text-lg font-black text-gray-900 dark:text-slate-100 text-center mb-2">
              {t("confirmAction")}
            </h3>
            <p className="text-sm text-gray-500 dark:text-slate-400 text-center mb-6 font-medium leading-relaxed">
              {deleteType === "old"
                ? `${t("confirmDeleteYear")} ${selectedYearToDelete}?`
                : t("confirmResetAll")}{" "}
              <br />
