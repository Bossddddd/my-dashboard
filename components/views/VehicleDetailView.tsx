  ...
  if (vehicleData === null) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h2 className="text-lg font-bold text-gray-800 dark:text-slate-200">ไม่พบข้อมูลยานพาหนะ</h2>
        <p className="text-gray-500 dark:text-slate-400 text-sm mt-1">ไม่พบเลขทะเบียนในระบบ</p>
        <button onClick={() => {setSearchInput(""); setVehicleData(undefined);}} className="mt-4 text-xs bg-gray-100 dark:bg-slate-800/50 text-gray-600 dark:text-slate-400 font-bold px-4 py-2 rounded-lg hover:bg-gray-200 dark:hover:bg-slate-600 transition-colors">กลับหน้าหลัก</button>
      </div>
    );
  }
  return (
    <div className="flex flex-col gap-6">
      <button onClick={() => {setSearchInput(""); setVehicleData(undefined);}} className="text-sm font-bold text-gray-500 dark:text-slate-400 hover:text-[#0B603A] flex items-center gap-1 transition-colors w-fit">← กลับหน้าแดชบอร์ดหลัก</button>
