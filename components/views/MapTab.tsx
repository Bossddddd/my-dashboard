...
const MultiMapComponent = dynamic(() => import('../MultiMapComponent'), { ssr: false });
...
export default function MapTab({
  ...
  const filteredLogs = useMemo(() => {
    return mapLogs.filter(log => {
      ...
      // Filter by overdue
      if (overdueOnly) {
        const isCompleted = log.status === 'completed' || log.status === 'cancelled';
    ...
    <div className="flex flex-col h-[calc(100vh-120px)] sm:h-[calc(100vh-140px)] gap-4">
      ...
      <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm flex flex-col gap-4 flex-shrink-0">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400 font-bold mt-1">
              พบข้อมูลพิกัด {stats.total} รายการ | <span className="text-rose-500">เกินกำหนด {stats.overdue} งาน</span> | <span className="text-amber-500">เร่งด่วน {stats.urgent} งาน</span>
            </p>
          </div>
        ...
        <div className="bg-gray-50 dark:bg-slate-900/50 p-3 rounded-xl border border-gray-100 dark:border-slate-700/50 flex flex-col xl:flex-row gap-4 xl:items-center">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {Object.entries(STATUS_CONFIG).map(([key, config]) => (
              <label key={key} className={`flex items-center gap-1.5 text-xs font-bold px-2.5 py-1.5 rounded-lg border cursor-pointer transition-colors ${selectedStatuses.includes(key) ? 'bg-white dark:bg-slate-800 text-[#0B603A] dark:text-emerald-400 border-[#0B603A]/30 dark:border-emerald-500/30 shadow-sm' : 'bg-transparent text-gray-400 border-transparent hover:bg-gray-200/50 dark:hover:bg-slate-800'}`}>
                ...
                />
                <div className={`w-3.5 h-3.5 rounded-sm border flex items-center justify-center transition-all ${selectedStatuses.includes(key) ? 'bg-[#0B603A] border-[#0B603A] dark:bg-emerald-500 dark:border-emerald-500' : 'bg-white border-gray-300 dark:bg-slate-800 dark:border-slate-600'}`}>
                  {selectedStatuses.includes(key) && <svg className="w-2.5 h-2.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}
                </div>
                {config.text}
              </label>
            ))}
          ...
          {/* Priority Checkboxes */}
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-[11px] uppercase tracking-wider font-bold text-gray-400 dark:text-slate-500 mr-1">เร่งด่วน:</span>
            {Object.entries(PRIORITY_CONFIG).map(([key, config]) => (
              <label key={key} className={`flex items-center gap-1.5 text-xs font-bold px-2.5 py-1.5 rounded-lg border cursor-pointer transition-colors ${selectedPriorities.includes(key) ? 'bg-white dark:bg-slate-800 text-amber-600 dark:text-amber-400 border-amber-500/30 dark:border-amber-500/30 shadow-sm' : 'bg-transparent text-gray-400 border-transparent hover:bg-gray-200/50 dark:hover:bg-slate-800'}`}>
                ...
                />
                <div className={`w-3.5 h-3.5 rounded-sm border flex items-center justify-center transition-all ${selectedPriorities.includes(key) ? 'bg-amber-500 border-amber-500' : 'bg-white border-gray-300 dark:bg-slate-800 dark:border-slate-600'}`}>
                  {selectedPriorities.includes(key) && <svg className="w-2.5 h-2.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}
                </div>
                {config.text}
              </label>
            ))}
      ...
      <div className="flex-1 bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm p-2 overflow-hidden relative">
        ...
        {mapLogs.length === 0 ? (
          ...
          <MultiMapComponent logs={filteredLogs} onMarkerClick={setActiveLogModal} />
        )}
      </div>
    </div>
  );
