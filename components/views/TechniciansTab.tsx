      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm flex flex-col md:flex-row items-center justify-center gap-8 lg:gap-16 w-full">
          <div className="flex flex-col items-center shrink-0">
            ...
            <PurePieChart
              success={tech.successCount}
              inProgress={tech.inProgressCount}
              late={tech.lateCount}
              size="lg"
            />
          </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden relative">
          ...
          <div className="p-4 bg-gray-50 dark:bg-slate-900 border-b border-gray-100 dark:border-slate-700/50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <h3 className="font-bold text-sm sm:text-base text-gray-800 dark:text-slate-200">
              📋 รายการประวัติใบงานซ่อมของช่างนี้ ({totalTechLogs} รายการ)
            </h3>
              ...
              {selectedIds.size > 0 && (
                <div className="flex gap-2 items-center bg-blue-50 dark:bg-blue-900/30 px-3 py-1.5 rounded-lg border border-blue-100">
                  ...
                  <ExportButton
                    ...
                    fileNamePrefix={`สรุปผลงานช่าง_${tech.name}`}
                  />
                </div>
              )}
              {selectedIds.size === 0 && (
                ...
                {currentLogs.map(
                  (
                    ...
                    return (
                      <tr
                        key={idx}
                        onClick={() => handleLogClick && handleLogClick(log)}
                        className={`hover:bg-blue-50 dark:bg-blue-900/30/30 cursor-pointer transition-colors ${selectedIds.has(logId as number) ? "bg-blue-50/80 dark:bg-blue-900/40" : ""}`}
                      >
                        <td
          ...
          <Pagination
            currentPage={currentTechLogPage}
            totalPages={logTotalPages}
            onPageChange={setCurrentTechLogPage}
            totalItems={totalTechLogs}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-emerald-200 dark:border-emerald-800 shadow-sm border-t-4 border-t-emerald-600">
          ...
          <div className="flex flex-col gap-3">
            {techsData.top.map(
              (
                techStat: import("../../lib/types").TechnicianStat,
                idx: number,
              ) => (
                <div
                  key={idx}
