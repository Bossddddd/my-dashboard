...
export default function TeamsTab({
  ...
  teamSums: { sumSuccess: number, sumInProgress: number, sumLate: number, totalteams: number };
  slaTarget: number;
  ...
  if (selectedteamDetail) {
    ...
    const logTotalPages = Math.max(1, Math.ceil(totalteamLogs / GENERAL_ITEMS_PER_PAGE));
    ...
    return (
      <div className="flex flex-col gap-6">
        <button onClick={() => { setSelectedteamDetail(null); setCurrentteamLogPage(1); }} className="text-sm font-bold text-gray-500 dark:text-slate-400 hover:text-[#0B603A] flex items-center gap-2 w-fit">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden relative">
          ...
          <div className="p-4 bg-gray-50 dark:bg-slate-900 border-b border-gray-100 dark:border-slate-700/50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <h3 className="font-bold text-sm sm:text-base text-gray-800 dark:text-slate-200">📋 รายการประวัติใบงานซ่อมของทีมช่างนี้ ({totalteamLogs} รายการ)</h3>
            <div className="flex flex-wrap items-center gap-2">
              {selectedIds.size > 0 && (
                <div className="flex gap-2 items-center bg-emerald-50 dark:bg-emerald-900/30 px-3 py-1.5 rounded-lg border border-emerald-100">
                  ...
                  <ExportButton selectedIds={Array.from(selectedIds)} fileNamePrefix={`สรุปผลงานทีมช่าง_${w.name}`} />
                </div>
              )}
              {selectedIds.size === 0 && (
            ...
            <table className="w-full text-left table-auto text-[11px] sm:text-sm min-w-[1000px]">
              ...
              <tbody className="divide-y divide-gray-100 bg-white dark:bg-slate-800">
                {currentLogs.map((log: import('../../lib/types').MaintenanceLog, idx: number) => {
                  ...
                  return (
                  <tr key={idx} onClick={() => setActiveLogModal(log)} className={`hover:bg-emerald-50 dark:bg-emerald-900/30/30 cursor-pointer transition-colors ${selectedIds.has(logId as number) ? "bg-emerald-50/80 dark:bg-emerald-900/40" : ""}`}>
          ...
          <Pagination currentPage={currentteamLogPage} totalPages={logTotalPages} onPageChange={setCurrentteamLogPage} totalItems={totalteamLogs} startIndex={logStartIndex} itemsPerPage={GENERAL_ITEMS_PER_PAGE} />
        </div>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-4 sm:gap-6">
      <div className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm flex flex-col md:flex-row items-center justify-center gap-6 sm:gap-12">
        <div className="flex flex-col items-center">
          <h3 className="text-sm sm:text-base font-black text-gray-800 dark:text-slate-200 mb-4 sm:mb-6 text-center">📊 สัดส่วนสถานะงานซ่อมรวมทุกทีมช่าง</h3>
          <PurePieChart success={teamSums.sumSuccess} inProgress={teamSums.sumInProgress} late={teamSums.sumLate} size="lg" />
        </div>
        <div className="flex flex-col gap-3 sm:gap-4 text-xs sm:text-sm font-bold bg-gray-50 dark:bg-slate-900 p-4 sm:p-6 rounded-xl border border-gray-100 dark:border-slate-700/50 w-full md:w-auto min-w-[250px] lg:min-w-[350px]">
          <div className="flex justify-between items-center pb-2 border-b border-gray-200 dark:border-slate-700"><span className="flex items-center gap-2 text-gray-600 dark:text-slate-400"><span className="w-3 h-3 rounded-full bg-emerald-500"></span>{t('successJobs')}</span><span className="text-emerald-600 text-xl font-black">{teamSums.sumSuccess}</span></div>
          <div className="flex justify-between items-center pb-2 border-b border-gray-200 dark:border-slate-700"><span className="flex items-center gap-2 text-gray-600 dark:text-slate-400"><span className="w-3 h-3 rounded-full bg-amber-500"></span>{t('inProgressJobs')}</span><span className="text-amber-500 text-xl font-black">{teamSums.sumInProgress}</span></div>
          <div className="flex justify-between items-center"><span className="flex items-center gap-2 text-gray-600 dark:text-slate-400"><span className="w-3 h-3 rounded-full bg-rose-500"></span>{t('lateJobs')}</span><span className="text-rose-600 text-xl font-black">{teamSums.sumLate}</span></div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-5 rounded-xl bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 shadow-sm border-l-4 border-l-[#0B603A]">
          <span className="text-gray-500 dark:text-slate-400 text-xs font-bold block mb-1">{t('totalteamsInSystem')}</span>
          <span className="text-3xl font-black text-[#0B603A]">{teamSums.totalteams} <span className="text-sm font-bold text-gray-400 dark:text-slate-500">{t('teamsUnit')}</span></span>
        </div>
        <div className="p-5 rounded-xl bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 shadow-sm border-l-4 border-l-blue-500">
          <span className="text-gray-500 dark:text-slate-400 text-xs font-bold block mb-1">ทีมช่างประสิทธิภาพ SLA ดีเยี่ยม (&gt;=80%)</span>
          <span className="text-3xl font-black text-blue-600">{stats.teamsData?.filter((w: import('../../lib/types').TeamStat) => w.efficiencyRate >= 80).length} <span className="text-sm font-bold text-gray-400 dark:text-slate-500">{t('teamsUnit')}</span></span>
        </div>
        <div className="p-5 rounded-xl bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 shadow-sm border-l-4 border-l-rose-500">
          <span className="text-gray-500 dark:text-slate-400 text-xs font-bold block mb-1">{t('lateteams')}</span>
          <span className="text-3xl font-black text-rose-600">{stats.teamsData?.filter((w: import('../../lib/types').TeamStat) => w.lateCount > 0).length} <span className="text-sm font-bold text-gray-400 dark:text-slate-500">{t('teamsUnit')}</span></span>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden mt-2">
        <div className="p-3 sm:p-5 border-b border-gray-100 dark:border-slate-700/50 bg-gray-50 dark:bg-slate-900 flex justify-between items-center">
          <h3 className="font-bold text-sm sm:text-base text-gray-800 dark:text-slate-200">🏢 ตารางทำเนียบรายชื่อทีมช่าง/ศูนย์บริการซ่อมบำรุง</h3>
        ...
        <div className="w-full overflow-x-auto">
          <table className="w-full text-left table-auto text-[11px] sm:text-sm min-w-[900px]">
            <thead className="text-gray-600 dark:text-slate-400 bg-gray-100 dark:bg-slate-800/50 border-b border-gray-200 dark:border-slate-700 text-[10px] sm:text-xs uppercase">
              <tr>
                <th onClick={() => handleSort("name")} className="p-2 sm:px-4 sm:py-3 font-bold cursor-pointer hover:bg-gray-200 dark:hover:bg-slate-600 whitespace-nowrap">ชื่อทีมช่าง {sortField === "name" ? (sortDirection === "asc" ? "▲" : "▼") : ""}</th>
            ...
            <tbody className="divide-y divide-gray-100 bg-white dark:bg-slate-800">
              {processedteams.map((team: any, index: number) => (
                <tr key={index} onClick={() => { setSelectedteamDetail(team); setCurrentteamLogPage(1); }} className="hover:bg-emerald-50 dark:bg-emerald-900/30/40 cursor-pointer transition-all">
                  <td className="p-2 sm:px-4 sm:py-4 font-black text-[#0B603A] hover:text-emerald-700 dark:text-emerald-400 flex items-center gap-1.5 whitespace-nowrap"><span className="text-sm sm:text-lg shrink-0">🏢</span> {team.name}</td>
                  <td className="p-2 sm:px-4 sm:py-4 text-center font-bold text-gray-800 dark:text-slate-200 whitespace-nowrap">{team.totalJobs}</td>
                  <td className="p-2 sm:px-4 sm:py-4 text-center text-emerald-600 font-black whitespace-nowrap">{team.successCount}</td>
                  <td className="p-2 sm:px-4 sm:py-4 text-center text-amber-500 font-black whitespace-nowrap">{team.inProgressCount}</td>
                  <td className="p-2 sm:px-4 sm:py-4 text-center text-rose-600 font-black whitespace-nowrap">{team.lateCount}</td>
                  <td className="p-2 sm:px-4 sm:py-4 text-center font-mono font-bold text-gray-600 dark:text-slate-400 whitespace-nowrap">{team.avgRepairHours} ชม.</td>
                  <td className="p-2 sm:px-4 sm:py-4 text-center whitespace-nowrap">
                    <span className={`inline-block px-1.5 sm:px-3 py-1 sm:py-1.5 rounded-lg font-mono font-black text-[9px] sm:text-sm shadow-xs ${
                      team.efficiencyRate >= (slaTarget || 80) ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' : team.efficiencyRate >= ((slaTarget || 80) - 30) ? 'bg-amber-50 dark:bg-amber-900/30 text-amber-700' : 'bg-rose-50 dark:bg-rose-900/30 text-rose-700 dark:text-rose-400'
                    }`}>{team.efficiencyRate}%</span>
                  </td>
                </tr>
              ))}
            </tbody>
