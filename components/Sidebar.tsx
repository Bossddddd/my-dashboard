...
interface SidebarProps {
  ...
  setActiveTab: (
    tab: "dashboard" | "teams" | "technicians" | "map" | "settings",
  ) => void;
}
      ...
      <div className="flex-1 overflow-y-auto py-4 flex flex-col gap-1 px-3">
        ...
        <div className="mt-2">
          ...
          >
            ...
            <svg
              className={`w-4 h-4 transition-transform ${isMaintenanceOpen ? "rotate-180" : ""}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <div className="mt-1 flex flex-col gap-1 pl-4">
              <div
                ...
                className={`px-3 py-2 text-sm font-bold rounded-lg border-l-4 cursor-pointer transition-all ${
                  activeTab === "dashboard"
                    ? "text-[#0B603A] bg-emerald-50 dark:bg-emerald-900/30 border-[#0B603A]"
                    : "text-gray-500 dark:text-slate-400 hover:text-gray-800 dark:text-slate-200 hover:bg-gray-50 dark:bg-slate-900 border-transparent"
                }`}
              >
...
function MenuItem({
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <span className={`text-lg ${isActive ? "" : "grayscale opacity-70"}`}>
          {icon}
        </span>
        <span>{label}</span>
      </div>
