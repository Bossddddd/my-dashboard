...
export default function DashboardSearchResults({
  ...
  return (
    <div className="flex flex-col gap-5 sm:gap-6">
      <button onClick={onReset} className="text-sm font-bold text-gray-500 dark:text-slate-400 hover:text-[#0B603A] flex items-center gap-1 transition-colors w-fit">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-gray-100">
            {results.workshops.map((workshop: any, idx: number) => (
              <button key={`${workshop.name}-${idx}`} onClick={() => onOpenWorkshop(workshop)} className="w-full p-4 text-left hover:bg-emerald-50 dark:bg-emerald-900/30/40 transition-colors flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                  <div className="font-black text-[#0B603A]">{workshop.name}</div>
                  <div className="text-xs text-gray-500 dark:text-slate-400 font-bold mt-1">งานทั้งหมด {workshop.totalJobs} | สำเร็จ {workshop.successCount} | ล่าช้า {workshop.lateCount}</div>
                </div>
                <span className="font-mono font-black text-sm text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 px-3 py-1 rounded-lg w-fit">SLA {workshop.efficiencyRate}%</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {results.technicians.length > 0 && (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
          <div className="p-4 bg-gray-50 dark:bg-slate-900 border-b border-gray-100 dark:border-slate-700/50">
            <h3 className="font-black text-sm text-gray-800 dark:text-slate-200">👨‍🔧 ช่างซ่อม</h3>
          </div>
          <div className="divide-y divide-gray-100">
            {results.technicians.map((technician: any, idx: number) => (
              <button key={`${technician.name}-${idx}`} onClick={() => onOpenTechnician(technician)} className="w-full p-4 text-left hover:bg-blue-50 dark:bg-blue-900/30/40 transition-colors flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                  <div className="font-black text-blue-800">{technician.name}</div>
                  <div className="text-xs text-gray-500 dark:text-slate-400 font-bold mt-1">งานทั้งหมด {technician.totalJobs} | สำเร็จ {technician.successCount} | ล่าช้า {technician.lateCount}</div>
                </div>
                <span className="font-mono font-black text-sm text-blue-700 bg-blue-50 dark:bg-blue-900/30 px-3 py-1 rounded-lg w-fit">SLA {technician.efficiencyRate}%</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {results.logs.length > 0 && (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
          <div className="p-4 bg-gray-50 dark:bg-slate-900 border-b border-gray-100 dark:border-slate-700/50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <h3 className="font-black text-sm text-gray-800 dark:text-slate-200">📋 ใบแจ้งซ่อม</h3>
            {results.logs.length > displayedLogs.length && (
              <span className="text-xs text-gray-400 dark:text-slate-500 font-bold">แสดง {displayedLogs.length} จาก {results.logs.length} รายการ</span>
            )}
          </div>
          <div className="divide-y divide-gray-100">
            {displayedLogs.map((log: import('../lib/types').MaintenanceLog, idx: number) => (
              <button key={`${log.maintenanceLogId || log.id}-${idx}`} onClick={() => onOpenLog(log)} className="w-full p-4 text-left hover:bg-amber-50 dark:bg-amber-900/30/40 transition-colors">
                ...
                <p className="text-sm text-gray-700 dark:text-slate-300 leading-relaxed line-clamp-2">{log.description}</p>
